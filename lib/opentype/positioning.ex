defmodule OpenType.Positioning do
  ...
  def apply_lookup({:parsed, type, flag, mfs, data}, gdef, lookups, isRTL, {glyphs, pos})
      when is_list(data) do
    Enum.reduce(data, {glyphs, pos}, fn subdata, input ->
      apply_lookup({:parsed, type, flag, mfs, subdata}, gdef, lookups, isRTL, input)
    end)
  end
  ...
  def apply_lookup({:parsed, 3, flag, mfs, data}, gdef, _lookups, isRTL, {glyphs, pos}) do
    ...
    g2 =
      ...
      |> Enum.reduce(glyphs, fn {x, i}, acc ->
        if Enum.at(acc, i).cursiveDelta == 0, do: List.replace_at(acc, i, x), else: acc
      end)
  ...
  def apply_lookup({:parsed, 4, flag, mfs, data}, gdef, _lookups, _isRTL, {glyphs, pos}) do
    ...
    g2 =
      d_glyphs
      |> Enum.reduce(glyphs, fn {x, i}, acc ->
        if Enum.at(acc, i).markDelta == 0, do: List.replace_at(acc, i, x), else: acc
      end)
    ...
    pos =
      case format do
        ...
        2 ->
          ...
          _positioning =
            applyContextPos2(coverage, rulesets, classes, gdef, lookups, glyphs, pos, [])
    ...
    values =
      case fmt do
        ...
        2 ->
          ...
          for <<(<<val::binary-size(valSize)>> <- recs)>>,
            do: Parser.readPositioningValueRecord(valueFormat, val)
      end
    ...
    kerning =
      case fmt do
        1 ->
          ...
          # parse the pair sets
          pairSets =
            Enum.map(pairsetOffsets, fn off ->
              Parser.parsePairSet(data, off, record1, record2)
            end)
        ...
        2 ->
          ...
          <<class1Off::16, class2Off::16, nClass1Records::16, nClass2Records::16,
            records::binary>> = rest
          ...
          pairSets =
            Enum.map(c1Recs, fn c2recs ->
              ...
              c2Recs
              |> Enum.map(fn c2Rec ->
                for <<(<<v1::binary-size(sizeA), v2::binary-size(sizeB)>> <- c2Rec)>>,
                  do: {v1, v2}
              end)
              |> Enum.map(fn [{v1, v2}] ->
                {Parser.readPositioningValueRecord(record1, v1),
                 Parser.readPositioningValueRecord(record2, v2)}
              end)
            end)
  ...
  def parse_lookup(3, data) do
    <<_fmt::16, coverageOff::16, nAnchorPairs::16, nrecs::binary-size(nAnchorPairs)-unit(32),
      _::binary>> = data

    coverage = Parser.parseCoverage(Parser.subtable(data, coverageOff))

    records =
      for <<(<<entryAnchor::16, exitAnchor::16>> <- nrecs)>>, do: {entryAnchor, exitAnchor}
    ...
    anchorPairs =
      ...
      |> Enum.map(fn {entryAnchor, exitAnchor} ->
        entryAnchor =
          if entryAnchor != 0,
            do: Parser.parseAnchor(Parser.subtable(data, entryAnchor)),
            else: nil

        exitAnchor =
          if exitAnchor != 0, do: Parser.parseAnchor(Parser.subtable(data, exitAnchor)), else: nil
  ...
  def parse_lookup(4, data) do
    <<_fmt::16, markCoverageOff::16, baseCoverageOff::16, nClasses::16, markArrayOffset::16,
      baseArrayOffset::16, _::binary>> = data
    ...
    baseArray =
      ...
      |> Enum.map(
        &Enum.map(&1, fn o ->
          Parser.parseAnchor(Parser.subtable(baseTbl, o))
        end)
      )
        ...
        3 ->
          ...
          btCoverage =
            Enum.map(backOffsets, fn covOff ->
              Parser.parseCoverage(Parser.subtable(data, covOff))
            end)
  ...
  defp applyMarkToLig(
    ...
    {mark_offset, delta} =
      if markloc != nil and baseloc != nil and !skipMark and g.mLigComponent > 0 do
  ...
  defp applyMarkToBase(
    ...
    {mark_offset, delta} =
      if markloc != nil and baseloc != nil and !skipMark do
        b = Enum.at(baseArray, baseloc)
        {class, {mark_x, mark_y}} = Enum.at(markArray, markloc)
  ...
  defp applyCursive(_coverage, _anchorPairs, _flag, _mfs, _gdef, _isRTL, prev, [], pos),
    do: {pos, prev}

  defp applyCursive(_coverage, _anchorPairs, _flag, _mfs, _gdef, _isRTL, prev, [x], pos),
    do: {pos, [x | prev]}
  ...
  defp applyCursive(
    ...
    # flag now ignored!
    # decompose the flag
    <<_attachmentType::8, _::3, _useMark::1, _ignoreMark::1, _ignoreLig::1, _ignoreBase::1,
      rtl::1>> = <<flag::16>>
      ...
      if curloc != nil and nextloc != nil do
        ...
        if exitA != nil and entryA != nil do
          ...
          if isRTL do
            ...
            [
              {:pos, xOff - delta_x, delta_y, xAdv - delta_x, yAdv},
            ...
            [
              {:pos, xOff, delta_y, exit_x + xOff, yAdv},
  ...
  # class-based context
  defp applyContextPos2(_coverage, _rulesets, _classes, _gdef, _lookups, [], [], output),
    do: output

  defp applyContextPos2(
         coverage,
         rulesets,
         classes,
         gdef,
  ...
  def should_skip_glyph(g, flag, gdef, mfs) do
    ...
    cond do
      ...
      ignoreBase == 1 and glyphClass == 1 ->
        true

      ignoreLig == 1 and glyphClass == 2 ->
        true

      ignoreMark == 1 and glyphClass == 3 ->
        true
      ...
      useMarkFilteringSet == 1 and classifyGlyph(g, gdef.classes) == 3 and
          findCoverageIndex(Enum.at(gdef.mark_sets, mfs), g) == nil ->
        true

      # skip if we don't match a non-zero attachment type
      attachmentType != 0 and classifyGlyph(g, gdef.attachments) != attachmentType ->
        true
