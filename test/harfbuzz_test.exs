...
defmodule GutenexHarfbuzzTest do
  ...
  def harfbuzz_test(font, text, expected) do
    ...
    ttf = OpenType.parse_file(filename)
  ...
  test "cursive tests (arabic shaping with GPOS 3)" do
    ...
    harfbuzz_test(
      "07f054357ff8638bac3711b422a1e31180bba863.ttf",
      # ARABIC CUBE ROOT, COMBINING MARK (DOTLESS HEAD OF KHAH)
      "\u0606\u06E1",
      "2@40,502+0|1+1000"
    )
  end
  ...
  test "mark filtering sets" do
    ...
    harfbuzz_test(
      "f22416c692720a7d46fadf4af99f4c9e094f00b9.ttf",
      "\u062A\u062E\u062A\u0629",
  ...
  test "arabic-like joining (positional shaping)" do
    ...
    harfbuzz_test(
      "5dfad7735c6a67085f1b90d4d497e32907db4c78.ttf",
